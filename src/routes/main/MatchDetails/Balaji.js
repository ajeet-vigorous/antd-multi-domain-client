...
const MatchDetailsBalaji = () => {
  ...
  const [betChips, setBetChips] = useState(() => {
    try {
      const storedBetChips = localStorage.getItem('betChipsData');
      if (storedBetChips) {
        return JSON.parse(storedBetChips);
      }
    } catch (error) {
      console.error('Failed to parse betChipsData from localStorage:', error);
    }
    return betChipsValues;
  });
  ...
  const columns = [
    {
      title: (
        ...
        >
          <div className="gx-fs-md gx-font-weight-bold gx-text-white">BOOKMAKER</div>
    ...
    {
      title: <div className="gx-fs-md gx-font-weight-bold gx-text-white">LAGAI</div>,
      dataIndex: "lgaai",
      key: "lgaai",
      width: "20%",
      align: "center",
    ...
    {
      title: <div className="gx-fs-md gx-font-weight-bold gx-text-white">KHAI</div>,
      dataIndex: "khaai",
      key: "khaai",
      width: "20%",
  ...
  const tiedColumn = [
    {
      title: (
        <div style={{ display: "flex " }} className="gx-bg-flex gx-justify-content-between minMax">
          <div className="gx-fs-md gx-font-weight-bold gx-text-white">TIED MATCH</div>
          <span style={{ textWrap: "nowrap" }} className="gx-fs-sm">
  ...
  const tossColumn = [
    {
      title: (
        <div style={{ display: "flex " }} className="gx-bg-flex gx-justify-content-between minMax">
          <div className="gx-fs-md gx-font-weight-bold gx-text-white">TOSS</div>
  ...
  useEffect(() => {
    if (!matchScoreDetails?.session) return;
    ...
    sorted.forEach((ele) => {
      if (ele.com_perm !== "YES") return;

      const currentType = getSessionType(ele.session_name);

      // Add section header row dynamically
      if (lastType !== currentType) {
        finalData.push({
          key: `header-${currentType}`,
          isHeader: true,
          session_name: `${currentType} SESSION`,
        });
        lastType = currentType;
      }

      // Add actual session row
      const record = {
        key: ele.session_id,
      ...
      if (record.remark) expandedKeys.push(record.key);
    });

    setData2(finalData);

    const lastHeader = finalData.find((r) => r.isHeader);
    if (lastHeader) setSessionTitle(lastHeader.session_name);
  ...
  const fancyColumns = [
    {
      ...
      render: (text, record) => {
        if (record.isHeader) {
          return {
            children: <div className="gx-fs-md gx-font-weight-bold gx-text-white">{text}</div>,
            props: { colSpan: 1, className: "gx-bg-grey", },
          };
        }
        return (
...

    {
      ...
      render: (text, record) => {
        if (record.isHeader) {
          return {
            children: <div className="gx-fs-md gx-font-weight-bold gx-text-white">NO</div>,
            props: { colSpan: 1, className: "gx-bg-grey", },
          };
        }
    ...
    },
    {
      ...
      render: (text, record) => {
        if (record.isHeader) {
          return {
            children: <div className="gx-fs-md gx-font-weight-bold gx-text-white">YES</div>,
            props: { colSpan: 1, className: "gx-bg-grey", },
          };
        }
        return record.running_status !== "SUSPENDED" ? (
      ...
      <Row justify={"center"}>
        ...
        <Col className="gx-px-0 gx-py-0 gx-mx-0 gx-my-0" xs={24} sm={24}>
          ...
          {data2 && data2.length > 0 ? (
            <Table
              className="gx-w-100 gx-mx-0 gx-my-0 gx-table-responsive ddddddddd"
              size="small"
              dataSource={data2}
            ...
            <div className="gx-my-1 gx-lg-block gx-d-none gx-d-lg-block gx-mx-1">
              <div className="gx-bg-flex gx-align-items-center gx-fs-xl  gx-px-5 ">
                ...
                >
                  ...
                  <span>
                    RATE : {betModalData?.runs ? betModalData?.runs : "0"} [{betModalData?.odds ? betModalData?.odds : 0}]
                    <span
                      style={{
                        color:
...
export default MatchDetailsBalaji;
